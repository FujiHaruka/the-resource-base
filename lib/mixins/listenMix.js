...
function listenMix (Class) {
  ...
  return class ListenMixed extends Class {
    ...
    listenToUpdate (onUpdate) {
      return this.listenEvents({
        [ENTITY_UPDATE]: ({id, updated, old}) => onUpdate(id, updated, {
          event: ENTITY_UPDATE,
          old
        }),
        [ENTITY_UPDATE_BULK]: ({ids, updated, old}) => ids.map((id, i) => onUpdate(id, updated[i], {
    ...
    listenToDestroy (onDestroy) {
      return this.listenEvents({
        [ENTITY_DESTROY]: ({id, destroyed, gone}) => (destroyed > 0) && onDestroy(id, destroyed, {
          gone,
          event: ENTITY_DESTROY
        }),
        [ENTITY_DESTROY_BULK]: ({ids, destroyed, gone}) => ids
          .map((id, i) => ({id: ids[i], destroyed: destroyed[i], gone: gone[i]}))
          .map(({id, destroyed, gone}) => (destroyed > 0) && onDestroy(id, destroyed, {
            gone: gone,
